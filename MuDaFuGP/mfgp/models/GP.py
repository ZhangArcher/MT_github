...
input_dim : 
 ...
 adapt_maximizer :
eps:
kernel: The Quadratic Exponential Kernel 
lower_bound:
upper_bound:
acquisition_obj:
...
class GP(object):
    ...
    def ARD(self, model):
        ...
        """
        gpflow.optimizers.Scipy.minimize(closure, variables, method='L-BFGS-B', 
        step_callback=None, compile=True, allow_unused_variables=False, **scipy_kwargs)
        
        Minimize closure.

        Minimize is a wrapper around the scipy.optimize.minimize function handling the packing 
        and unpacking of a list of shaped variables on the TensorFlow side vs. the flat numpy array required on the Scipy side.  
        
        Parameters:
            closure (Callable[[], Tensor]) – 
                A closure that re-evaluates the model, returning the loss to be minimized.
            variables (Sequence[Variable]) – 
                The list (tuple) of variables to be optimized (typically model.trainable_variables)
        """
    ...
    def predict_opt(self, X, X_test, ard=False):
        ...
        method:
            predict_f(Xnew, full_cov=False, full_output_cov=False)
                This method computes predictions at X in input points
